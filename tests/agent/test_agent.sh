...
export DON_TOOLS_DIR="$SCRIPT_DIR/tools"
CONFIG_FILE="test_agent"  # Will look for test_agent.yaml in DON_TOOLS_DIR
LOG_FILE="$TESTS_ROOT/agent_test_output.log"
TEST_NAME="test_agent"

# Model resolution:
# 1. Use DON_AGENT_MODEL if set
# 2. Otherwise, let agent use default from config file
MODEL_FLAG=""
if [ -n "$DON_AGENT_MODEL" ]; then
    MODEL_FLAG="--model $DON_AGENT_MODEL"
fi
...
info "Testing MCPShell agent with config: $CONFIG_FILE (using DON_TOOLS_DIR=$DON_TOOLS_DIR)"
...
if [ $CHECK_RESULT -eq 0 ]; then
    ...
    warning ""
    warning "Example: DON_AGENT_MODEL=qwen3:14b OPENAI_API_BASE=http://localhost:11434/v1 ./test_agent.sh"
    warning "═══════════════════════════════════════════════════════════════════"
    warning ""
...
info "Model: ${DON_AGENT_MODEL:-default from config}"
